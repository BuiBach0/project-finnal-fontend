...
export const registerMutationFn = async (data: registerType): Promise<void> => {
  try {
...
export const TaskMutationFn = async ({
  workspaceId,
  projectId,
  data,
}: CreateTaskPayloadType) => {
...
export const getTaskByIdQueryFn = async ({
  ...
  projectId,
  taskId,
}: TaskByIdPayloadType): Promise<TaskApiResponse> => {
  const response = await API.get(
...
export const editTaskMutationFn = async ({
  ...
  taskId,
  data,
}: EditTaskPayloadType): Promise<TaskResponseType> => {
  const response = await API.put(
